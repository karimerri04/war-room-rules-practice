...
class IncidentControllerMockMvcTest {
	...
	void shouldReturnAllIncidents() throws Exception {
		mockMvc.perform(get("/api/java-incidents")).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$", hasSize(3))).andExpect(jsonPath("$[0].id").exists())
				.andExpect(jsonPath("$[0].title").exists()).andExpect(jsonPath("$[0].severity").exists())
	...
	void shouldReturnIncidentById() throws Exception {
		mockMvc.perform(get("/api/java-incidents/JAVA-INC-001")).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
	...
	void shouldReturnNotFoundWhenIncidentDoesNotExist() throws Exception {
		mockMvc.perform(get("/api/java-incidents/UNKNOWN")).andExpect(status().isNotFound())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
	...
	void shouldResolveIncident() throws Exception {
		...
		mockMvc.perform(patch("/api/java-incidents/JAVA-INC-001/resolve").contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.id").value("JAVA-INC-001")).andExpect(jsonPath("$.status").value("RESOLVED"))
	...
	void shouldReturnBadRequestWhenIncidentIsAlreadyResolved() throws Exception {
		...
		mockMvc.perform(patch("/api/java-incidents/JAVA-INC-001/resolve").contentType(MediaType.APPLICATION_JSON)
				.content(firstRequestBody)).andExpect(status().isOk());

		mockMvc.perform(patch("/api/java-incidents/JAVA-INC-001/resolve").contentType(MediaType.APPLICATION_JSON)
				.content(secondRequestBody)).andExpect(status().isBadRequest())
	...
	@Test
	@DisplayName("GET /api/java-incidents/stats should return incident dashboard statistics")
	void shouldReturnIncidentStats() throws Exception {
		mockMvc.perform(get("/api/java-incidents/stats")).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.total").value(3)).andExpect(jsonPath("$.open").value(3))
				.andExpect(jsonPath("$.investigating").value(0)).andExpect(jsonPath("$.resolved").value(0))
